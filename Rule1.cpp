...
void analyzeVHDLCode(const std::string &vhdlCode)
{
    ...
    while (pos < vhdlCode.length())
    {
        ...
        // int f = KMPSearch("pc_comb",constructContent);
        // condition=removeSpaces(condition);
        vector<int> idx;
        for (int i = 0; i < condition.size(); i++)
        {
            if (condition[i] == '=') idx.push_back(i);
        }
        int f=0;
        for(auto i:idx)
        {   
            ...
            // cout<<temp<<endl;
            // if(!check_binary(temp)){
            //     f=1;
            // }
            // if(condition[i+3]=='1' or condition[i+3]=='0' ){
            //     f=0;
            // }
        ...
        // while (std::regex_search(searchStart, s.cend(), matches, assignmentRegex))
        // {
        //     // std::cout << "Found assignment: " << matches[0] << "\n";
        //     // std::cout << "Signal: " << matches[1] << ", Value: " << matches[2] << "\n";
        //     if(matches.str(1)=="pc_comb(15 downto 8)" || matches.str(2)=="pc_comb(7 downto 0)")
        //     {
        //         f=1;
        //     }
