...
void analyzeVHDLCode(const std::string &vhdlCode)
{
    ...
    while (pos < vhdlCode.length())
    {
        ...
        {
            if (idx[i]+3<condition.length() && (condition[idx[i] + 2] == '1' or condition[idx[i] + 2] == '0'))
            {
                if (condition[idx[i] + 3] != '1' and condition[idx[i] + 3] != '0')
                {
                    f = 1;
                    break;
                }
...
int main()
{
    std::string inputFileName = "RULE2_control_mem_rtl_T200.vhd";        // Replace with your input file name
